...
function VInfo:LMBDown()
	...
	if (VInfo.settings.canMoveText) then return; end;
...
function VInfo:action_viSetMoveText(actionName, keyStatus, arg3, arg4, arg5)
	VInfo.settings.canMoveText = not VInfo.settings.canMoveText;
end;


-- Set the vehicle parking
function VIData:togglePark(park)
	if (self.vi == nil) then return; end;

	if (park == type("boolean")) then self.vi.isParked = park;
	else self.vi.isParked = not self.vi.isParked;
	end;

	if (self.spec_motorized.isMotorStarted and self.vi.isParked) then
		if (self.deactivateLights ~= nil) then self:deactivateLights();	end;
		if (self.stopMotor ~= nil) then	self:stopMotor(); end;
