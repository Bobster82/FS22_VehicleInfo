...
function VInfo:updateObjectText(info, lineNumber)
	...
	for index, obj in pairs(info) do
		if (type(obj) == "table") then
			if (obj.text ~= nil) then
				...
				if (index == 1) then
					text = obj.text;
					objWidth = (string.len(text) < 8) and size*2 or getTextWidth(size, text);
				else
					text = obj.text;
					if (string.len(text) > VInfo.settings.maxTextLength) then
						text = string.sub(text, 0, VInfo.settings.maxTextLength);
						text = text.."..";
					end;
				...
				if (VInfo.settings.canMoveText) then setTextColor(unpack(VInfo.colors.blocked));
				elseif (obj.color) then setTextColor(unpack(obj.color));
				...
				-- Render the seperator if not last item
				if (index ~= #info) then
	...
	local bg = Overlay.new(VInfo.images["BG_VInfo"], VInfo.text.posX-0.001, VInfo.text.posY - (size * lineNumber) - (size/4.5), lineWidth+0.001, (size));
...
function VInfo:getFillUnitInfo(vehicle, fillUnitNum)
	...
	if math.floor(vehicle:getFillUnitFillLevel(fillUnitNum)) > 0 then
		local fruit = nil;
		if vehicle.spec_fillUnit.fillUnits[fillUnitNum] ~= nil then
			...
			fillUnit = string.format("%s(%s%%), %s", math.floor(vehicle:getFillUnitFillLevel(fillUnitNum)), fillPercent, fruit);
		end;
	end;
...
function VInfo:setFillColor(vehicle, fillPercent)
	...
	if (not vehicle.spec_sowingMachine and not vehicle.spec_sprayer) then
		if (vehicle.brand.title == "Hardi") then
			-- its a Hardi extra tank for the sprayer
		...
		else-- Probably a trailer or harvester
...
function VInfo:getAttachedInfo(vehicle, obj)
	if VInfo:getAllAttachedImplements(vehicle) ~= nil then
		...
		for i = #allImp, 1, -1 do
			...
			if not (allImp[i].spec_workArea or allImp[i].spec_fillUnit) then goto skipToNext; end;  -- 
			maxImp = maxImp + 1;
...
function VInfo:isVehicleBlocked(vehicle)
	...
	if (vehicle.vi.moveTimer == nil) then vehicle.vi.moveTimer = 5000; end;
	if vehicle.spec_aiVehicle.isActive then
		if (vehicle:getLastSpeed() < 2 and not VInfo:getIsWaiting(vehicle)) then
			vehicle.vi.moveTimer = vehicle.vi.moveTimer-10;
		else
			vehicle.vi.moveTimer = 5000;
		end;
		if (vehicle.vi.moveTimer < 0) then return true; end;
	else
		vehicle.vi.moveTimer = nil;
	end;
	return false;
end;
...
function VInfo:changeObjColor(obj, color)
	for k,v in pairs(obj) do
		if (type(v) == "table") then
			if obj[k].color == self.colors.entered then
			elseif obj[k].color == self.colors.lowFuel then
			else obj[k].color = color;
			end
		end;
	end;
end;
...
function VInfo:getIsWaiting(vehicle)
	if (vehicle == nil) then return false; end;
	if (vehicle:getLastSpeed() > 2) then return false; end;

	if vehicle.ad ~= nil then
		if 	vehicle.ad.stateModule:getCurrentTaskInfo() == "$l10n_AD_task_wait_for_call;" or 
			string.find(vehicle.ad.stateModule:getCurrentTaskInfo(), "AD_task_drive_to") and vehicle.ad.drivePathModule:isTargetReached() or
			vehicle.ad.trailerModule:isActiveAtTrigger() then 
			return true
		end
	end

	if VInfo:getIsCpFieldWorker(vehicle) then return false; end; -- then its not a cp ai combine unloader
	if VInfo:getIsCpUnloader(vehicle) then return true; end;

	return false;
end

-- Returns if Courseplay is active
function VInfo:getIsCPActive(vehicle)
	if (vehicle.getIsCpActive and vehicle:getIsCpActive()) then return true; end;
	return false;
end

-- Returns if Courseplay fieldwork is active or not
function VInfo:getIsCpFieldWorker(vehicle)
	if (vehicle.spec_cpAIFieldWorker) then 
		if (vehicle:getIsCpFieldWorkActive()) then return true; end;
	end;
	return false;
end

-- Returns if Courseplay AI Unloader is active or not
function VInfo:getIsCpUnloader(vehicle)
	if not (vehicle.spec_cpAIFieldWorker) then return false; end;
	if not (vehicle.spec_cpAIFieldWorker.driveStrategy) then return false; end;
	if (vehicle.spec_cpAIFieldWorker.driveStrategy.isACombineUnloadAIDriver) then return true; end;
	
	return false;
end

function VInfo:getIsLowFuel()
	if self ~= nil and self.spec_motorized then
		local lowLevel = 0.20 -- percent
		local fuel = self:getConsumerFillUnitIndex(FillType.DIESEL)
		local level = self:getFillUnitFillLevel(fuel)
		local cap = self:getFillUnitCapacity(fuel)
		if type(level) =="number" and type(cap) == "number" and level/cap < lowLevel then
			return true
		end
	end
	return false
...
function VInfo:getActiveName(vehicle, isEntered, isHelper, name, Obj)
	if (isEntered) then
		local txt = vehicle:getControllerName() or "Player";
		table.insert(Obj, name, {text = txt});
		Obj[name].bold = true;
	elseif (isHelper) then
		local txt;
		if (VInfo:getIsCPActive(vehicle)) then txt = VInfo.text.cpHelper;
		elseif (vehicle.ad ~= nil and vehicle.ad.stateModule:isActive()) then txt = VInfo.text.adHelper; end;
		txt = txt or VInfo.text.helper;
		table.insert(Obj, name, {text = txt});
		Obj[name].bold = true;
		if (self.settings.showHelperName) then
			if (vehicle:getCurrentHelper().name ~= nil) then
				txt = vehicle:getCurrentHelper().name;
				table.insert(Obj, name+1, {text = txt});
			end;
		end;
	end;
end;

function VInfo:getIsLeasedIsMission(vehicle, Obj)
	if (vehicle.propertyState == 2) then -- Leased vehicle
		local index = #Obj+1;
		Obj[index] = {};
		Obj[index].text = VInfo.text.isLeased;
		Obj[index].color = VInfo.colors.isLeased;
		Obj[index].bold = true;
	elseif (vehicle.propertyState == 3) then -- Mission vehicle
		local index = #Obj+1;
		Obj[index] = {};
		Obj[index].text = VInfo.text.isMission;
		Obj[index].color = VInfo.colors.isLeased;
		Obj[index].bold = true;
	end;
end;
